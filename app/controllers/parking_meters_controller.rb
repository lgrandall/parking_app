class ParkingMetersController < ApplicationController
  ...
  def active_parking_meters
    @parking_meters = ParkingMeter.all.where({status: 'on'})
  end 

  def inactive_parking_meters
    @parking_meters = ParkingMeter.all.where({status: 'off'})
  end

  def out_of_order_parking_meters
    @parking_meters = ParkingMeter.all.where({status: 'out_of_order'})
  end
